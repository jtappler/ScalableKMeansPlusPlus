...
12.001063375006197
    ...
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{ScalableKMeansPlusPlus_files/ScalableKMeansPlusPlus_16_1.png}
    \end{center}
    { \hspace*{\fill} \\}
...
\end{Verbatim}
    ...
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{ScalableKMeansPlusPlus_files/ScalableKMeansPlusPlus_21_0.png}
    \end{center}
    { \hspace*{\fill} \\}
...
\end{Verbatim}
    ...
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{ScalableKMeansPlusPlus_files/ScalableKMeansPlusPlus_24_0.png}
    \end{center}
    { \hspace*{\fill} \\}
...
1.8935244909953326
    ...
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{ScalableKMeansPlusPlus_files/ScalableKMeansPlusPlus_31_1.png}
    \end{center}
    { \hspace*{\fill} \\}
...
function is significantly faster than the navie KMeansParallel function.
(move than three times faster)} The total time was improved from 13.4s
to 3.66s.

    \subsection{Optimization: Cytonized
Version}\label{optimization-cytonized-version}

    From the profiling, we can see most system time and resources were using
in step 3: looping. Thus if we rewrite the distance function using C,
should be able to significantly improve the efficiency.

    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}109}]:} \PY{o}{\PYZpc{}}\PY{k}{load\PYZus{}ext} cythonmagic
\end{Verbatim}
...
The cythonmagic extension is already loaded. To reload it, use:
  \%reload\_ext cythonmagic
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}111}]:} \PY{o}{\PYZpc{}\PYZpc{}}\PY{k}{cython} \PYZhy{}a
          import numpy as np
          cimport numpy as np
          ...
          def cminimum\PYZus{}distance(np.ndarray[np.float64\PYZus{}t, ndim=1] p, 
                                np.ndarray[np.float64\PYZus{}t, ndim=2] centers):
              
              cdef int min\PYZus{}index = 0
              cdef float min\PYZus{}dist = 1.79769313486e+308
              cdef int i
              cdef np.ndarray[np.float64\PYZus{}t, ndim=1] cc
              for i, cc in enumerate(centers):
                  d = np.sum((p \PYZhy{} cc) ** 2)
                  if min\PYZus{}dist \PYZgt{} d:
                      min\PYZus{}dist = d 
                      min\PYZus{}index = i
              return (min\PYZus{}dist,min\PYZus{}index)
\end{Verbatim}
